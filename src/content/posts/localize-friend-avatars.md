...
我的友链页面（`/friends/`）有 9 个友链，头像来源五花八门：
...
最终效果：8 张静态头像压到合计约 128KB，动图保持原样，页面不再依赖任何外部站点。
...
6. 清理目录里「配置中已彻底不再引用」的旧文件（已本地化的头像不会被误删）。
...
**「清理逻辑会误删已本地化的头像」**。脚本的「清理不再被引用的旧文件」最初只把「本次新下载的头像」当作要保留的，导致只要某次构建里有新友链要下载，之前已经本地化的头像全被当成「未引用」删掉，而配置里还引用着本地路径，页面就裂图了。修复：清理前先收集配置里所有已是 `/images/friends/` 的 `imgurl`，只删除「既不是本次新下载、配置里也不再引用」的文件，脚本这才真正幂等。
...
脚本是**幂等**的：全部头像已经是本地路径时直接跳过，什么都不改（清理逻辑只删配置中已不再引用的文件，不会误删已本地化的头像）；单个头像下载失败也不会中断构建，只是警告并保留原链接兜底。所以每次 `npm run build` 都会自动检查一遍，加新友链后构建一次就自动本地化了。

## 效果

- 友链页 9 个头像全部走 `/images/friends/`，不再依赖对方站点速度；
- 8 张静态头像合计约 128KB（原图 1.7MB+）；
- 动态头像正常播放动画；
